...
VertexShaderOutput ShadowMapVertexShader(VertexShaderInput input)
{
	...
	output.Position = mul(mul(mul(input.Position, World), LightViewMatrix),
...
float4 ShadowMapPixelShader(VertexShaderOutput input) : COLOR0
{
	...
	float depth = 1.0 - projTexCoord.z; // for shadow map
...
float4 ShadowedScenePixelShader(ShadowedSceneVertexShaderOutput input) : COLOR0
{
		...
		saturate(projTexCoord).y == projTexCoord.y)
	{
		...
		if (realDistance + (1.0f/100.f) > depthStoredInShadowMap) // "1.0f/100.f" is bias
		{
			diffuseLightingFactor = max(0, dot(N,L)); //Gray No Shadow
		}
		else
		{
			diffuseLightingFactor = float4(1,0,0,1); //Red Shadow Casting
		}
	}
